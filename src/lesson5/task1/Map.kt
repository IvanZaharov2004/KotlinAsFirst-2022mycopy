...
fun buildGrades(grades: Map<String, Int>): Map<Int, List<String>> {
    ...
    for ((student, grade) in grades) {
        when (grade) {
            in res -> res.getOrDefault(grade, mutableListOf()).add(student)
            else -> res += grade to mutableListOf(student)
        }
    }
    return res
}
...
fun mergePhoneBooks(mapA: Map<String, String>, mapB: Map<String, String>): Map<String, String> {
    ...
    for (entry in mapB) {
        if (mapA.containsKey(entry.key) && mapA[entry.key] != entry.value) {
            val newValue = mapA[entry.key] + ", ${entry.value}"
            res[entry.key] = newValue
        } else {
...
fun extractRepeats(list: List<String>): Map<String, Int> {
    ...
    for (word in list)
        if (word !in result.keys) result[word] = 1
        else result[word] = result[word]!! + 1
...
fun findSumOfTwo(list: List<Int>, number: Int): Pair<Int, Int> {
    ...
    for (i in list.indices) map += list[i] to i
    for ((index, value) in list.withIndex()) {
        if (number - value in map && map[number - value] != index) return (map[number - value]!! to index).sorted()
    }
    return -1 to -1
}
/**
